...
class ApplicationController < ActionController::Base
  ...
  #----------------------------------------------------------------------------
  def auto_complete
    @query = params[:auto_complete_query] || params[:term] || ''
  ...
  def auto_complete_rails4
    ...
    respond_to do |format|
      format.any(:js, :html)   { render partial: 'auto_complete' }
      format.json do
        render json: @auto_complete.inject([]){|r, a|
                       new_item = {}
                       new_item[:id] = a.id
                       new_item[:label] = a.respond_to?(:full_name) ? h(a.full_name) : h(a.name)
                       r << new_item
                     }, :root => false
      end
    end
